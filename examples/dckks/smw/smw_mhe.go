...
func mergePartyCiphertexts(params ckks.Parameters, encInputs []*ckks.Ciphertext, evaluator ckks.Evaluator) *ckks.Ciphertext {
	...
	for len(encInputs) > 1 {
		...
		tmpEncInputs = make([]*rlwe.Ciphertext, 0)
		sumGroupSize = 0
...
func main() {
	...
	encRes.Scale *= float64(len(expRes) * len(P)) //each element contains the average value
	...
	evaluator.InnerSum(encResDeviation, 1, params.Slots(), encResDeviation)
	...
	//summation results: 必须有outCiphertext := pcksPhase(params, tpk, inCiphertext, P) // cpk -> tpk否则结果错误
	fmt.Println("Summation:")
	//summation
	ptresSummation := ckks.NewPlaintext(params, params.MaxLevel())
	for i, encInputCopy2 := range encInputsCopy2 {
		evaluator.InnerSum(encInputCopy2, 1, params.Slots(), encInputCopy2)
		encInputCopy2 = pcksPhase(params, tpk, encInputCopy2, P)          //*若无此行，则结果错误
		decryptor.Decrypt(encInputCopy2, ptresSummation)                  //ciphertext->plaintext
		resSummation := encoder.Decode(ptresSummation, params.LogSlots()) //plaintext->complex numbers
		fmt.Printf("Summation of Party[%d]=%.6f\t", i, real(resSummation[0]))
		fmt.Println()
	}
...
func encPhase(params ckks.Parameters, P []*party, pk *rlwe.PublicKey, encoder ckks.Encoder) (encInputs, encInputsCopy, encInputsCopy2 []*rlwe.Ciphertext) {

	l := log.New(os.Stderr, "", 0)

	encInputs = make([]*rlwe.Ciphertext, len(P))
	encInputsCopy = make([]*rlwe.Ciphertext, len(P))
	encInputsCopy2 = make([]*rlwe.Ciphertext, len(P))

	for i := range encInputs {
		encInputs[i] = ckks.NewCiphertext(params, 1, params.MaxLevel())
		encInputsCopy[i] = ckks.NewCiphertext(params, 1, params.MaxLevel())
		encInputsCopy2[i] = ckks.NewCiphertext(params, 1, params.MaxLevel())
	}
...
func evalPhase(params ckks.Parameters, NGoRoutine int, encInputs []*rlwe.Ciphertext, evaluator ckks.Evaluator) (encRes *rlwe.Ciphertext) {
