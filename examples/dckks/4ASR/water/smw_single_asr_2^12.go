...
var encryptedSectionNum = 1 // from 0 to 60
...
func process(householdIDs []int, maximumLenPartyRows int, folderName string, params ckks.Parameters) {
	...
	fmt.Printf("Nothing: %v, %v, %v", len(expSummation), len(expDeviation), len(plainSum))
	...
	for i, _ := range encSummationOuts {
		...
		fmt.Printf("Nothing: %v", resSummation[0])
		// fmt.Printf("CKKS Summation of Party[%d]=%.6f\t", i, real(resSummation[0])+plainSum[i])
		// fmt.Printf(" <===> Expected Summation of Party[%d]=%.6f\n", i, expSummation[i])
	}
	...
	for i, _ := range encDeviationOuts {
		...
		// fmt.Printf(" <===> Expected Average of Party[%d]=%.6f\n", i, expAverage[i])
		...
		fmt.Printf("Nothing: %v", delta)
		// fmt.Printf("CKKS Deviation of Party[%d]=%.6f\t", i, real(resDeviation[0])-delta)
		// fmt.Printf(" <===> Expected Deviation of Party[%d]=%.6f\n", i, expDeviation[i])
	}
