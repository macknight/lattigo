...
func genInputs(params ckks.Parameters, P []*party, maximumLenPartyRows int) (globalPartyRows int, expSummation, expAverage, expDeviation, plainSum []float64) {
	...
	for pi, po := range P {
		...
		for i := range po.rawInput {
			...
			if i%sectionSize > 0 {
				if !almostEqual(po.rawInput[i], po.rawInput[i-1]) {
					po.flag[i/sectionSize] += 1
				}
			}
		}
		...
		for i := range po.flag {
			po.group[po.flag[i]] += 1
		}
		for randNumber > 0 {
			min := sectionNum + 1
			transitions := -1
			for index := 0; index < sectionSize; index++ {
				if po.group[index] < min {
					min = po.group[index]
					transitions = index
				}
			}
			...
			for index := 0; index < sectionNum; index++ {
				if po.flag[index] == transitions {
					po.flag[index] = -1 //label for selective encryption
					flagCount++
				}
			}
			randNumber--
