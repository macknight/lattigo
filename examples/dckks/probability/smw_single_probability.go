...
const sectionSize = 4 // element number within a section
...
type party struct {
	...
	group                        []int
	entropy                      []float64
	entropyReductionPerOperation []float64
}
...
func genInputs(params ckks.Parameters, P []*party, maximumLenPartyRows int) (globalPartyRows int, expSummation, expAverage, expDeviation, plainSum []float64) {
	...
	for pi, po := range P {
		...
		po.entropyReductionPerOperation = make([]float64, sectionNum) //2160 sections
		...
		for i := range po.rawInput {
			...
			if i%sectionSize == sectionSize-1 {
		...
		for i := 0; i < sectionNum; i++ {
			...
			po.flag[index] = 1
			entropyReduction += max
			po.entropyReductionPerOperation[i] = entropyReduction / float64(i+1)
		}
