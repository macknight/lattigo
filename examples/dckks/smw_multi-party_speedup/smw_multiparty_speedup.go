...
func main() {
	...
	// paramsDef := ckks.PN18QP7660CI
	paramsDef := ckks.PN12QP109CI
	...
	householdIDs := []int{4} // using suffix IDs of the csv files
	...
	for i, r := range res {
		if i < visibleNum || (i > len(expRes)-visibleNum && i < 1+len(expRes)) {
