...
func process(fileList []string, params ckks.Parameters) {
	...
	fmt.Printf(">>>>>>>Water Entropy Histograme:\n")
...
func markEncryptedSections(P []*party, threshold, entropySum float64, transitionSum int) {
	...
	encryptedSectionNum = int(math.Floor(float64(sectionNum) * threshold)) //encryptedSectionNum == sectionNum
	for i := 0; i < encryptedSectionNum; i++ {
		previousEntropySum := entropySum
		previousTransitionSum := transitionSum

		for _, po := range P {
			r := getRandom(encryptedSectionNum - i)
			index := po.flag[r]
			entropyReduction := po.entropy[index]
			transitionReduction := po.transition[index]
			po.flag[r] = po.flag[encryptedSectionNum-1-i]
			po.flag[encryptedSectionNum-1-i] = index
