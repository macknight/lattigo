...
var encryptedSectionNum = 50 // from 0 to 60
...
func genInputs(params ckks.Parameters, P []*party, maximumLenPartyRows int) (globalPartyRows int, expSummation, expAverage, expDeviation, plainSum []float64) {
	...
	for pi, po := range P {
		...
		plainInputSize := lenPartyRows - encryptedSectionNum*sectionSize
		if plainInputSize < 0 {
			plainInputSize = 0
		}
		po.plainInput = make([]float64, plainInputSize) //plaintext needed
		...
		for i := range po.rawInput {
			...
			if i%sectionSize == 0 {
				if i/sectionSize < encryptedSectionNum {
					po.input[i/sectionSize] = make([]float64, sectionSize)
				}
			}

			//prepare po.input for encryption & po.plainInput
			if i/sectionSize < encryptedSectionNum {
				po.input[i/sectionSize][i%sectionSize] = po.rawInput[i]
			} else {
				po.plainInput[i-encryptedSectionNum*sectionSize] = po.rawInput[i]
			}
		}
		...
		for i := 0; i < lenPartyRows; i++ {
			if po.flag[i/sectionSize] == -1 {
				po.input[i/sectionSize] = append(po.input[i/sectionSize], po.rawInput[i])
			} else {
